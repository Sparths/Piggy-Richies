...
* Scatter count for pays/triggers is read from the freshly-drawn board of each
  spin (not from tiles that tumble in mid-cascade).
...
class GameState:
    ...
    def run_spin(self, bet_mode_name: str = "base") -> dict:
        ...
        scatters, scatter_positions = self._count_scatters()

        win, _ = self._play_cascades(
    ...
    def run_freespin(self, *, start_level: int, start_bricks: int, spins: int) -> float:
        ...
        while spins_remaining > 0 and total < self.config.wincap:
            ...
            # Collect brick tokens on reel 5 (index 4) and apply any upgrade.
            for row in range(self.config.num_rows):
                if self.board.grid[4][row] == "BR":
                    bricks += 1
                    es.collect_brick(bricks, [4, row])
            ...
            # Re-stamp persistent sticky wilds from earlier spins.
            if sticky_active:
            ...
            spin_win, end_index = self._play_cascades(
    ...
    def _count_scatters(self):
        positions = self.board.positions_of(self.scatter)
